...
The pattern matches `godot-bridge/cli/cmd/godot-bridge/main.go` (see `writeJSON`, `writeJSONLine`). Copy the shape, not the exact code — this repo has no transport layer, so it can be simpler.
