...
- **godot-bridge integration (bridgex).** Deferred indefinitely. The tool writes `.tres` files to disk; the user (or their agent) imports them manually or via their own wiring. Keeps sprite-gen decoupled from any specific editor-control tool.
...
| 02 | CI + Releases | Lock in build/test/release before accumulating code. Mirrors `godot-bridge` patterns. Second-smallest PR. | `02-ci-releases.md` |
...
- No dependencies on `godot-bridge` at build time or runtime
