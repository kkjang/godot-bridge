...
Lock in automated build/test/release infrastructure before accumulating feature code. Mirrors the patterns in `godot-bridge/.github/workflows/` but simplified for a single-component repo (sprite-gen has no plugin or LSP side).
...
Tag format: `vX.Y.Z` (plain semver — no module path prefix, since this repo is a single module at its root, unlike `godot-bridge` which has three submodules).
...
- "Release only after CI passes on the default branch" is the same guard `godot-bridge` uses.
- Unlike `godot-bridge`, we do not need `component:` labels — there's one component and it's the whole repo.
...
(File path: `.github/release.yml` — the `-sprite-gen` suffix was a copy-paste from `godot-bridge`'s multi-component naming; drop the suffix here since this repo has only one release stream.)
...
Single-component mirror of the godot-bridge release pattern: CI on
