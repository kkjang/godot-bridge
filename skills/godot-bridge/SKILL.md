...
- Treat `gdscript-lsp-proxy` as transport that OpenCode should start from the configured `opencode.json` when `.gd` LSP support is needed, not as a process to launch manually from `bash` during normal work.
...
2. If `opencode.json` configures `gdscript-lsp-proxy` for `.gd`, OpenCode should start that proxy when LSP support is needed; use OpenCode's regular built-in LSP interface instead of launching the proxy from `bash` yourself or creating ad-hoc JSON-RPC/TCP LSP clients, unless the user is explicitly debugging LSP setup.
...
- In normal work, if `opencode.json` configures `gdscript-lsp-proxy`, OpenCode should start it when `.gd` LSP support is needed; then use OpenCode's built-in LSP operations for `.gd` files rather than launching `gdscript-lsp-proxy` from `bash` yourself.
